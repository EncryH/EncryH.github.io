...
header:
  teaser: "/image/2026-07-27-DAH2026-preliminary/attack-agent-architecture.png"
---
...
나는 공격 체인 마지막 단계인 Follow-Up Attack Agent를 맡았다. 쉽게 말하면 정찰팀이 정보를 모아오고, 분석팀이 취약점을 찾으면, 내가 그걸 가지고 실제로 공격 이벤트를 쏘는 역할이다.

# 2. 도메인 이해 — 방산 무인체계

가장 먼저 한 건 방산 무인체계가 실제로 어떻게 돌아가는지 이해하는 거였다. UAV가 혼자 나는 게 아니라 지상통제체계(GCS), 전술통신망(TMMR/TICN), 위성망까지 얽힌 구조라는 걸 파악하는 데서 시작했다.

![방산 무인체계 운용 환경 개념도 — UAV/UGV, 지상통제, 전술통신망의 연동 구조](/image/2026-07-27-DAH2026-preliminary/defense-uav-concept.png)

이 구조를 보면서 공격 포인트를 찾기 시작했다. 핵심은 MAVLink 프로토콜이었다. UAV-GCS 간 통신을 담당하는 이 프로토콜에 구조적 문제 두 가지가 있었다.
...
방향이 잡히고 나서 바로 환경 구축에 들어갔다. 당연히 진짜 UAV를 날릴 수는 없다. Docker Compose로 방산 무인체계 통신 구조를 컨테이너로 재현했다.

처음에는 UAV, 정찰, 공격, 방어 4개 컨테이너로 시작했는데, 실제 방산 통신 구조를 반영하려면 계층이 더 필요했다. 최종적으로 12개 컨테이너가 됐다.
...
테스트베드가 올라간 다음, 공격 에이전트를 설계했다. 3단계 체인으로 나눴다.
...
내 담당인 Follow-Up Attack 단계에서 만든 모듈은 두 가지다.

**JammerAdapter** — 전술 라우터(TMMR/TICN)의 링크 품질을 떨어뜨리는 시뮬레이션이다. 벡터 두 개를 만들었다.
...
급격한 저하 버전을 먼저 만들고 나서 "이거 방어 에이전트가 너무 쉽게 잡는데?" 하고 느끼고 은닉 버전을 추가했다. 공격을 만들어봐야 방어의 허점이 보인다는 걸 직접 체감했다.

**TamperAdapter** — MAVLink 프레임을 일부러 비정상으로 합성해서, 방어 쪽이 이걸 제대로 탐지하는지 테스트하는 용도다. Fail-safe 유도보다는 공격 표면 검증 성격이 강하다.

# 5. 방어 에이전트 — 4-Agent 구조

공격 에이전트를 돌려보니 방어가 허술한 부분이 보였다. 방어는 공격과 독립적으로 돌아가는 4단계 구조로 잡았다.
...
Kill Chain을 돌리면 5개 벡터가 순서대로 시도된다. 실행 전략은 `fallback_until_success` — 앞에서 성공하면 끝, 실패하면 다음 벡터로 넘어간다.
...
기본은 dry-run이다. 실제 이벤트를 날리려면 `--execute` 플래그랑 `ENABLE_LAB_ATTACKS=true` 환경변수가 둘 다 있어야 한다.
...
1~3번(MAVLink 계열)이 다 실패하고 4번 `EW_LINK_DEGRADATION_SIM`에서 성공했다. Mission State가 `FAILSAFE_LANDED`로 바뀌고 UAV 고도가 0m로 내려간 걸 볼 때 좀 짜릿했다.
...
방어 쪽도 incident report가 남는다. 탐지 → 대응 → 복구 타임라인이 JSON으로 기록된다.
...
코드가 돌아가기 시작하자 보고서 작성에 들어갔다. 7월 5일에 8섹션 구조를 확정했다.

```text
시나리오 개요 → 운용 환경 → 공격 목표 → 위협 모델 → 공격 표면 → 공격 전개 → 판정 기준 → 예상 피해
```

7월 7일, 마감 3일 전. 이 시점에 돌아가는 공격 벡터가 C-1(EW 링크 저하) 하나뿐이었다. "일단 돌아가는 거 먼저, 안 돌아가면 보고서도 못 쓴다"는 원칙으로 C-1에 집중했는데, 쓰다 보니 MAVLink 인증 부재 계열도 구현할 수 있겠다는 판단이 들어서 결국 5개 벡터를 다 만들었다.

코드가 돌아가니까 보고서에 넣을 게 생기고, 보고서를 쓰다 보니 빠진 구현이 보이고, 그걸 채우면 보고서가 더 탄탄해지는 순환이 생겼다.
...
참고문헌은 15편. MAVLink Developer Guide, ArduPilot Failsafe 문서, KISA 드론 보안 가이드부터 MAVSec 논문, NIST CSF 2.0까지 달았다.
...
대회 규정상 GPL은 못 쓰고 MIT, Apache 2.0, BSD만 허용이다. 코드 IP는 팀 귀속이지만 LIG D&A에 무상 비독점 실시권을 부여하는 조건이라, 라이브러리 하나 가져다 쓸 때마다 라이선스를 확인했다.

# 9. 돌아보면

솔직히 모듈들 만들면서 제일 많이 고민한 건 "이게 의미가 있나?"였다. Docker 안에서 숫자 조작하는 건데 현실적인 공격이라고 할 수 있나? 근데 결론은, 공격 메커니즘이 아니라 **공격이 성공했을 때 시스템이 어떻게 반응하는지**를 보는 게 핵심이었다. Fail-safe가 제대로 걸리는지, 방어 에이전트가 잡아내는지, 그걸 검증하는 테스트베드로서 의미가 있었다.
